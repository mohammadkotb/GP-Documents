...
categorization is presented. The machine learning approach for text 
...
and different types of classifiers are presented. In section \ref{sec:related_work}
related work to Smart Email is presented with features of each paper. Then, a comparison
between related applications will be outlined in section \ref{sec:related_apps}. And
summarizing the need to extend related applications is explained in section
...
engineering effort goes towards the construction not of a classiﬁer, 
but of an automatic builder of classiﬁers (the learner). \cite{Sebastiani2002}
...
Gmail priority inbox is an algorithmic-based filtering system that predicts 
which messages are most important to the recipient and displays them atop the inbox queue.

Gmail uses a variety of signals to identify important email, including which 
messages the user opens and which messages he replies to.
...
\begin{center}
  \begin{table}[H]
    \begin{tabular}{ | p{3cm} | c | c | c | p{2cm} |}
      \hline
...
Close study of table \ref{related_applications_summary} reveals the need to extend all existing applications to cover the desirable features enumerated below:
\begin{enumerate}
