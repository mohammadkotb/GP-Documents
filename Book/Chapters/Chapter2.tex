...
Probabilistic classifiers view CSV$_{i}$(d$_{j}$) in terms of P(c$_{i}$ | d$_{j}$), i.e. the probability that a document d$_{j}$ belongs to c$_{i}$, and compute this probability by an application of Bayes’ theorem, given by \[ P(c_{i}|d_{j}) = \frac{P(c_{i})P(d_{j}|c_{i})}{P(d_{j})} \]
